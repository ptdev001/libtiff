...
_TIFFVSetField(TIFF* tif, uint32_t tag, va_list ap)
{
    ...
    case TIFFTAG_GRAYRESPONSECURVE:
      uv16 = (uint16)(1L << td->td_bitspersample);
      _TIFFsetShortArray(&td->td_grayresponsecurve,
						va_arg(ap, uint16*), uv16);
      break;
...
_TIFFVGetField(TIFF* tif, uint32_t tag, va_list ap)
{
	...
	switch (standard_tag) {
		case TIFFTAG_GRAYRESPONSECURVE:
      *va_arg(ap, uint16**) = td->td_grayresponsecurve;
		case TIFFTAG_SUBFILETYPE:
...
TIFFFreeDirectory(TIFF* tif)
{
	...
	CleanupField(td_grayresponsecurve);
