...
struct tiff
{
  ...
  #define TIFF_LAZYSTRILELOAD                                                                                       \
    0x2000000U /* lazy/ondemand loading of strip/tile offset/bytecount values. Only used if TIFF_DEFERSTRILELOAD is \
  ...
  #define TIFFhowmany_32(x, y)                                                                                        \
    (((uint32_t)x < (0xffffffff - (uint32_t)(y - 1))) ? ((((uint32_t)(x)) + (((uint32_t)(y)) - 1)) / ((uint32_t)(y))) \
                                                      : 0U)
  ...
  /* Safe multiply which returns zero if there is an *unsigned* integer overflow. This macro is not safe for *signed*
   * integer types */
...
extern "C"
{
  ...
  extern tmsize_t _TIFFReadEncodedStripAndAllocBuffer(
    TIFF* tif, uint32_t strip, void** buf, tmsize_t bufsizetoalloc, tmsize_t size_to_read);
  extern tmsize_t _TIFFReadEncodedTileAndAllocBuffer(
    TIFF* tif, uint32_t tile, void** buf, tmsize_t bufsizetoalloc, tmsize_t size_to_read);
  extern tmsize_t _TIFFReadTileAndAllocBuffer(
    TIFF* tif, void** buf, tmsize_t bufsizetoalloc, uint32_t x, uint32_t y, uint32_t z, uint16_t s);
