...
{                      /* state table entry */
  ...
  uint32_t
    Param; /* unsigned 32-bit run length in bits (holds on 16 bit actually, but cannot be changed. See above warning) */
} TIFFFaxTabEnt;
  ...
  "TermW  ", "TermB  ", "MakeUpW", "MakeUpB", "MakeUp ", "EOL    ",
    ...
    #define LOOKUP8(wid, tab, eoflab)                                                                   \
      ...
      {                                                                                                 \
        ...
        printf("%08lX/%d: %s%5d\t", (long)BitAcc, BitsAvail, StateNames[TabEnt->State], TabEnt->Param); \
    ...
    #define SETVALUE(x)                                                                                       \
      ...
      {                                                                                                       \
        ...
        {                                                                                                     \
          TIFFErrorExt(tif->tif_clientdata, module, "Buffer overflow at line %u of %s %u", sp->line,          \
                       isTiled(tif) ? "tile" : "strip", isTiled(tif) ? tif->tif_curtile : tif->tif_curstrip); \
          return (-1);                                                                                        \
        }                                                                                                     \
        *pa++ = RunLength + (x);                                                                              \
        a0 += (x);                                                                                            \
